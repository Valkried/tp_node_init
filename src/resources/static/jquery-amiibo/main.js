$(function () {
    ...
    const distantServerPath = "http://127.0.0.1:" + serverPort;
      ...
      $.get(urlAPI, function (amiiboFullArray) {
        ...
        $("nav").on("click", "li", function () {
          ...
          const filteredAmiibo = amiiboFullArray.amiibo.filter(
            ...
            (amiibo) =>
              amiibo[$(this).parent().attr("data-filter")] === $(this).text()
          );
  
          $(".title").html(
            $(this).parent().parent().find("h2").text() + " / " + $(this).text()
          );
          ...
          filteredAmiibo.forEach((amiibo, index) => {
            ...
            $(`#img_${$(this).parent().attr("id")}_${index}`).bind(
              "load",
              function () {
                $(this).fadeIn(1000);
              }
            ...
            $(`#img_${$(this).parent().attr("id")}_${index}`).mouseenter(
              function () {
                $(this).addClass("change");
              }
            );
            // Remove effect when mouse leave on image
            $(`#img_${$(this).parent().attr("id")}_${index}`).mouseleave(
              function () {
        ...
        const localFullListUrl =
          "http://127.0.0.1:8888/assets/amiibo-local/amiibo-array.json";
